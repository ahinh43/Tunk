...
async def on_member_update(before, after):
    ...
    currentTime = time.strftime('%H:%M:%S')
    if before.server.id == '159184581830901761':
        if before.server_permissions.administrator == False and after.server_permissions.administrator == True:
            ...
            for role in after.roles:
                if role not in before.roles and role in after.roles:
                    ...
                    break
            await client.send_message(client.get_channel(loggingchannel), '`[' + str(currentTime) + ']` :exclamation: ' + f'{after.name} has been granted **Administrator** powers.\nLast role changed: {differenceRole}')
        elif str(after.server_permissions.administrator) == 'False' and str(before.server_permissions.administrator) == 'True':
            loggingchannel = '477335474525175830'
            differenceRole = 'None'
            for role in before.roles:
                if role not in after.roles and role in before.roles:
                    differenceRole = role.name + f' (ID: {role.id})'
                    break
            await client.send_message(client.get_channel(loggingchannel), '`[' + str(currentTime) + ']` :exclamation: ' + f'{after.name} has had their **Administrator** powers removed.\nLast role changed: {differenceRole}')
    ...
    #adminRole = discord.utils.get(client.get_server('159184581830901761').roles, id='254972564906967042')
    if before.nick == after.nick:
        return

    # if adminRole in after.roles and adminRole not in after.roles:
    #     loggingchannel = '477335474525175830'
    #     await client.send_message(client.get_channel(loggingchannel), '`[' + str(currentTime) + ']` :exclamation: ' + f'{after.name} has been given the **Administrator** role.')
    # elif adminRole in before.roles and adminRole not in after.roles:
    #     loggingchannel = '477335474525175830'
    #     await client.send_message(client.get_channel(loggingchannel), '`[' + str(currentTime) + ']` :exclamation: ' + f'{after.name} has had the **Administrator** role removed from them.')  
    else:
...
async def on_server_role_update(before, after):
    if before.server.id == '159184581830901761':
        ...
        if before.permissions.administrator == False and after.permissions.administrator == True:
            loggingchannel = '477335474525175830'
            await client.send_message(client.get_channel(loggingchannel), '`[' + str(currentTime) + ']` :exclamation: ' + f'The role **{after.name}** has the **Administrator** permission added.')
        if before.permissions.administrator == True and after.permissions.administrator == False:
            loggingchannel = '477335474525175830'
            await client.send_message(client.get_channel(loggingchannel), '`[' + str(currentTime) + ']` :exclamation: ' + f'The role **{after.name}** has the **Administrator** permission removed.')
@client.event
async def on_ready():
    connectedServers = 0
    print('Logged in as')
